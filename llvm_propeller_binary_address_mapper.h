...
struct BbHandleBranchPath {
  ...
  friend void AbslStringify(Sink &sink, const BbHandleBranchPath &path) {
    absl::Format(&sink, "BBHandleBranchPath[pid:%lld, branches:%s", path.pid,
                 absl::StrJoin(path.branches, ", "));
...
// functions with profiles in `lbr_aggregation`. If `lbr_aggregation == nullptr`
// all functions will be included. Does not take ownership of `lbr_aggregation`
...
absl::StatusOr<std::unique_ptr<BinaryAddressMapper>> BuildBinaryAddressMapper(
    ...
    PropellerStats &stats, const LbrAggregation *lbr_aggregation = nullptr);
